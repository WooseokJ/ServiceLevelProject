...
class AuthView: BaseView {
    ...
    lazy var authTextLabel: UILabel = { // 휴대폰 소개 라벨
        let label = UILabel()
        label.text = "\(AuthText.textLabel.rawValue)"
    ...
    lazy var authredirectButton: UIButton = { //재전송 버튼
        ...
        button.setTitle(AuthText.textReDirectButton.rawValue, for: .normal)
        return button
    }()
    
    lazy var authButton: UIButton = { // 인증문자 버튼
        let button = UIButton()
        button.setTitle("\(AuthText.textButton.rawValue)", for: .normal)
