...
final class APIService {
    ...
    func searchRequest(lat: Double, long: Double, completionHandler: @escaping SearchInfo) {
        ...
        currentUser?.getIDTokenForcingRefresh(true) { idToken, error in
            ...
            print("갱신한 idToken",idToken)
            UserDefaults.standard.set(idToken!, forKey: "token")
            let api = API.search(lat: lat, long: long)
            
            AF.request(api.url, method: .post, parameters: api.parameters, headers: api.headers).validate().responseDecodable(of: Search.self) { response in
                print("========================")
